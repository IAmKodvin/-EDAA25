...
typedef struct poly_t poly_t; //empty struct, creates nickname for struct poly_t
...
//Pushes node to the end NOT USED?
...
void test3(void){
	char a[25] = "x^3 - 2x^2"; //"4x^5 + 2x^3"; //"5x^4000 - 8x^2 + 6";
	poly_t*	p;	

	p = new_poly_from_string("x^3 - 2x^2");
	print_poly(p);
	free_poly(p);
}
